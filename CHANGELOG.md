...
- **Crush trace and agent-launch reads stream message-by-message**
  — `Adapter.Parse` and `readAgentLaunches` now consume
  `go-crush-data.IterMessages` instead of materialising the full
  message history, so huge sessions no longer spike memory on the
  read path. Cross-message tool_call/tool_result pairing already
  resolved through the pending/results maps, so traces are
  unchanged; fixture, stress, and race suites pin the behaviour.
