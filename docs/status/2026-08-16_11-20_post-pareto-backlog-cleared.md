...
- **Use `//nolint:` with a reason, not silence.** Every new nolint directive in this session explains _why_ the rule doesn't apply (project's in-package test convention, test loopback, intentional complex integration test). Future agents can grep `//nolint` and learn from the rationale.
