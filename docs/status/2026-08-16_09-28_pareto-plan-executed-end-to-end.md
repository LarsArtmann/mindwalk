...
The headline find: **the plan's T06 fix would not have surfaced the actual bug on its own.** Adding the T07 regression test exposed a _second_ latent bug (cross-message tool_result pairing) that had silently downgraded every Crush session's error observability grade. The plan's T07 deliverable ("regression test") forced me to write a test that crashed the test suite — which is exactly what regression tests are for.
