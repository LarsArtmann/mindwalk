import { r, j as t, a as rs, R as Qe, c as as } from "./react-gcHzaSmV.js";
import {
  C as jt,
  V as te,
  Q as ze,
...
const oe = (e, n) => {
  const s = r.forwardRef(({ className: a, ...o }, l) =>
    r.createElement(ys, {
      ref: l,
      iconNode: n,
      className: gn(`lucide-${ms(zt(e))}`, `lucide-${e}`, a),
      ...o,
    }),
  );
  return ((s.displayName = zt(e)), s);
};
const ws = [
...
const zs = [
  ...
  Nt = oe("loader", zs);
const Ts = [["path", { d: "m8 3 4 8 5-5 5 15H2L8 3z", key: "otkl63" }]],
  xn = oe("mountain", Ts);
const Fs = [
...
function pr({ panels: e, openSheet: n, openPop: s, onToggle: a, onClosePop: o }) {
  ...
  return (
    r.useEffect(() => {
      ...
      const c = (w) => {
          const I = w.target;
          l.current?.contains(I) || f.current?.contains(I) || o();
        },
        S = (w) => {
          w.key === "Escape" && o();
        };
...
function mr({
  ...
  return t.jsxs("div", {
    ...
    children: [
      ...
      S ? t.jsx(Bt, { error: S, locked: o, onRetry: v }) : null,
      t.jsxs("div", {
              ...
              r.Fragment,
              {
                children: [
                  t.jsx(gr, {
                    ...
                    onPreview: ($) =>
                      I((C) =>
                        C?.mode === "pinned" ? C : { agentID: p.id, mode: "preview", anchor: $ },
                      ),
                    onPreviewEnd: ($) =>
                      I((C) => (C?.agentID === $ && C.mode === "preview" ? null : C)),
                    onToggleDetails: ($) =>
                      I((C) =>
                        C?.agentID === p.id && C.mode === "pinned"
                          ? null
                          : { agentID: p.id, mode: "pinned", anchor: $ },
                      ),
                  }),
                  j ? t.jsx(Bt, { error: j, locked: o, onRetry: v, rowLocal: !0 }) : null,
                ],
              },
              p.id,
            );
      ...
      w && O ? t.jsx(xr, { agent: O, state: w, onClose: () => I(null) }) : null,
    ],
  });
}
    ...
    r.useLayoutEffect(() => {
      const m = () => {
          const b = Math.min(520, window.innerWidth - 24),
            ...
            T = Math.min(o.current?.offsetHeight ?? O, O),
            p = Math.min(Math.max(c.top, 12), window.innerHeight - T - 12);
          f({ left: I, top: p, width: b, maxHeight: O });
        },
...
function Mr({ status: e, analyzing: n, locked: s, onAnalyze: a, onClose: o, onJumpTo: l }) {
  const [f, m] = r.useState(br),
    ...
    b = r.useCallback((S) => {
      m(S);
      try {
        localStorage.setItem(jn, JSON.stringify(S));
      } catch {}
    }, []),
  ...
  return new Promise((S, w) => {
    ...
    const p = () => {
        (I && cancelAnimationFrame(I),
          d.getTracks().forEach((P) => P.stop()),
          l?.removeEventListener("abort", C));
      },
      j = () => {
        if (T) return;
        ((T = !0), p());
        const P = b.mimeType || v || "video/webm";
        S({ blob: new Blob(c, { type: P }), extension: zr(P) });
      },
      $ = (P) => {
        if (!T) {
          ((T = !0), p());
      ...
      C = () => $(new Error("Recording cancelled."));
    if (
      ((b.ondataavailable = (P) => {
        P.data && P.data.size > 0 && c.push(P.data);
      }),
      (b.onerror = () => $(new Error("Recording failed."))),
      (b.onstop = j),
      l)
    ) {
      if (l.aborted) {
    ...
    const L = (P) => {
      if (T) return;
      O || (O = P);
      const ne = Math.min(1, (P - O) / m),
        K = Math.min(s - 1, Math.floor(ne * (s - 1)));
      if ((a(K), o?.(ne), ne >= 1)) {
    ...
    (a(0), b.start(), (I = requestAnimationFrame(L)));
  });
}
function Dr(e, n) {
  ...
  updateTargets(n, s, a) {
    ...
    for (const d of this.labels) {
      ...
      const w = ((this.point.x + 1) / 2) * s,
        I = ((1 - this.point.y) / 2) * a;
      if (
        !(this.point.z < 1 && w > -60 && w < s + 60 && I > -40 && I < a + 40) ||
        S < Ur ||
        S > f * 1.6
      ) {
        d.target = 0;
      ...
      (d.sprite.scale.set(p * d.aspect, p, 1),
        m.push({ label: d, sx: w, sy: I, pw: T * d.aspect, ph: T }));
    }
...
function An(e, n, s, a, o, l) {
  ...
  const v = s.clone().project(e),
    ...
    b = ((1 - v.y) / 2) * o,
    c = 48,
    S = Math.max(c + 60, a - l - 48),
    w = 120,
    I = o - 100,
    O = Math.min(Math.max(d, c), S),
    T = Math.min(Math.max(b, w), I);
  if (O === d && T === b) return;
  const p = Math.tan(kt.degToRad(e.fov) / 2),
    j = p * e.aspect,
    $ = new te().crossVectors(f, new te(0, 1, 0)).normalize(),
    C = new te().crossVectors($, f),
    L = $.multiplyScalar(((d - O) * 2 * m * j) / a).addScaledVector(C, ((T - b) * 2 * m * p) / o);
  (e.position.add(L), n.target.add(L), n.update());
}
class Ln {
  constructor(n) {
    ((this.host = n),
...
function Qr({
  ...
  const K = r.useRef(null),
    G = r.useMemo(() => {
      ...
      for (const i of e.files)
        ((A = Math.min(A, i.rect.x)),
          (x = Math.max(x, i.rect.x + i.rect.w)),
          (k = Math.min(k, i.rect.z)),
          (E = Math.max(E, i.rect.z + i.rect.d)));
      return {
        cx: (A + x) / 2,
        cz: (k + E) / 2,
        size: Math.max(x - A, E - k, 60),
        halfW: (x - A) / 2,
        halfD: (E - k) / 2,
      };
    }, [e]);
  return (
    r.useEffect(() => {
      ...
      L.current = x;
      const k = new an();
      ((k.background = it), (O.current = k));
      const E = new on(38, A.clientWidth / A.clientHeight || 1, 0.1, 2400);
      (E.position.set(70, 130, 100), (w.current = E));
      const i = new cn({ antialias: !0 });
      (i.setPixelRatio(Math.min(window.devicePixelRatio, 2)),
        i.setSize(A.clientWidth, A.clientHeight),
        (S.current = i),
        A.appendChild(i.domElement),
        o?.(i.domElement));
      const h = new ln(E, i.domElement);
      ((h.enableDamping = !x),
      ...
      k.add(y);
      const R = new un("#b6c5de", 1.1);
      (R.position.set(-60, 120, -40), k.add(R));
      const q = new cs(),
        U = new ls(),
        de = (X) => {
          if (!w.current || !S.current) return;
          const J = S.current.domElement.getBoundingClientRect();
          ((U.x = ((X.clientX - J.left) / J.width) * 2 - 1),
            (U.y = -((X.clientY - J.top) / J.height) * 2 + 1),
            q.setFromCamera(U, w.current));
          const he = [v.current, m.current].filter(Boolean),
            ie = q.intersectObjects(he, !1)[0];
          if (!(!ie || ie.instanceId === void 0)) {
            if (ie.object === v.current) {
              const H = b.current[ie.instanceId];
              return H ? d.current[H.fileId] : void 0;
            }
          ...
          z ||
            (z = requestAnimationFrame(() => {
              ...
              const he = ne.current,
                ...
                H = he.visitsByFile.get(J.id) ?? 0,
                N = ie ? `${Xe(ie)} · ${H} visit${H === 1 ? "" : "s"}` : Xe(void 0);
      ...
      (i.domElement.addEventListener("pointerdown", le),
        ...
        i.domElement.addEventListener("pointermove", D),
      ...
      const ue = new mn(),
        ...
        je = () => {
          (h.update(),
            $.current?.updateTargets(E, i.domElement.clientWidth, i.domElement.clientHeight),
            $.current?.ease(L.current));
          const X = v.current,
            J = b.current,
            he = c.current;
          if (X && J.length > 0) {
            ...
            for (let N = 0; N < J.length; N++) {
              const _ = J[N],
                V = d.current[_.fileId];
              if (!V) continue;
              let ee = he.get(_.fileId) ?? 0;
              const ce = _.target - ee;
              Math.abs(ce) > 0.015
                ? ((ee = L.current ? _.target : ee + ce * 0.13), he.set(_.fileId, ee), (H = !0))
                : ee !== _.target && (he.set(_.fileId, _.target), (ee = _.target), (H = !0));
              const fe = Math.max(V.rect.w, 0.45) + 0.04,
                Z = Math.max(V.rect.d, 0.45) + 0.04;
              (se.compose(
                new te(
                  V.rect.x + V.rect.w / 2 - P.current.cx,
                  Math.max(ee, 0.02) / 2 + Ge,
                  V.rect.z + V.rect.d / 2 - P.current.cz,
                ),
                xe,
            ...
            H && (X.instanceMatrix.needsUpdate = !0);
          }
          const ie = j.current;
          if (ie?.visible) {
            const H = ue.getElapsedTime(),
              N = ie.userData.baseScale,
              _ = L.current ? 1 : 1 + 0.1 * Math.sin(H * 2.4);
            ie.scale.setScalar(N * _);
          }
          (i.render(k, E), (C.current = requestAnimationFrame(je)));
        };
      return (
        je(),
        () => {
          (C.current && cancelAnimationFrame(C.current),
            ...
            i.domElement.removeEventListener("pointermove", D),
            i.domElement.removeEventListener("pointerleave", Y),
            g.dispose(),
            W.disconnect(),
            h.dispose(),
            i.dispose(),
            A.removeChild(i.domElement),
            k.clear(),
            o?.(null));
        }
      );
    ...
    r.useEffect(() => {
      ...
      const x = new hn(),
        k = G.size;
      A.fog = new fn(it, k * 2.1, k * 4.2);
      const E = new We(new wt(k * 6, k * 6), new nt({ color: "#14171e", roughness: 1 }));
      ((E.rotation.x = -Math.PI / 2), (E.position.y = -0.32), x.add(E));
      const i = new pn(k * 2.8, 46, "#20242e", "#1a1e27");
      ...
      if (h.length > 0) {
        const F = new bt(1, 1, 1),
          z = new nt({ roughness: 0.95, metalness: 0 }),
          D = new qe(F, z, h.length),
          Y = new _e(),
          re = new me();
        (h.forEach((W, ue) => {
          const se = -0.2 + Math.min(W.depth, 3) * 0.06,
            xe = se + 0.3;
          (Y.compose(
            new te(W.rect.x + W.rect.w / 2 - G.cx, se - xe / 2, W.rect.z + W.rect.d / 2 - G.cz),
            new ze(),
            new te(W.rect.w, xe, W.rect.d),
          ),
            D.setMatrixAt(ue, Y),
            re.set("#1a1f29").lerp(new me("#252b37"), Math.min(W.depth, 3) / 3),
            D.setColorAt(ue, re));
        }),
      ...
      const g = new bt(1, 1, 1),
        ...
        R = new qe(g, y, e.files.length),
        q = new _e();
      for (const F of e.files) {
        const z = Math.max(F.rect.w, 0.45),
          D = Math.max(F.rect.d, 0.45),
          Y = F.rect.x + F.rect.w / 2 - G.cx,
          re = F.rect.z + F.rect.d / 2 - G.cz;
        (q.compose(new te(Y, Ge / 2, re), new ze(), new te(z, Ge, D)),
          R.setMatrixAt(F.id, q),
          R.setColorAt(F.id, Ht(F)));
      }
      ((R.instanceMatrix.needsUpdate = !0),
        R.instanceColor && (R.instanceColor.needsUpdate = !0),
        (m.current = R),
        x.add(R));
      const U = new qe(ea(), new Be({ toneMapped: !1, vertexColors: !0 }), e.files.length);
      ((U.instanceColor = new st(new Float32Array(e.files.length * 3), 3)),
        (U.count = 0),
        (U.frustumCulled = !1),
        (v.current = U),
        x.add(U),
      ...
      ((de.userData.baseScale = Math.max(k * 0.028, 2.2)),
        (de.visible = !1),
        (j.current = de),
        x.add(de));
      const ae = new zn(1.4);
      ((p.current = ae), x.add(ae.object), (T.current = x), A.add(x));
      const le = () => {
        const F = w.current,
          z = I.current;
        if (!F || !z) return !0;
        const D = new te(0.46, 1.08, 0.72).normalize(),
          Y = [];
        for (const ue of [-1, 1])
          for (const se of [-1, 1]) Y.push(new te(ue * G.halfW, 0, se * G.halfD));
        const re = $n(F, D, Y);
        if (re === null) return !1;
        const W = Math.max(re * 1.12, k * 0.6);
        return (
          F.position.copy(D).multiplyScalar(W),
          z.target.set(0, 0, 0),
          (z.minDistance = k * 0.18),
          (z.maxDistance = Math.max(k * 2.6, W * 1.2)),
          z.update(),
          !0
        );
      };
      return (
        (K.current = le() ? null : le),
        () => {
          ((K.current = null),
            In(x),
            A.remove(x),
    ...
    r.useEffect(() => {
      ...
      for (const g of e.files) {
        const y = n.touchByFile.get(g.id),
          R = g.path === s;
        if ((x.setColorAt(g.id, R ? Ce.selected : Ht(g)), y)) {
          const q = n.visitsByFile.get(g.id) ?? 1;
          let U = Ce[y];
          (g.ghost && (U = U.clone().lerp(Ce.ghost, 0.45)),
            R && (U = Ce.selected),
            E.push({ fileId: g.id, target: Dt(y, q), color: U }),
            i.add(g.id));
        } else if (l) {
          const q = Xr(g.lines, h);
          let U = Zr(q);
          (g.ghost && (U = U.lerp(Ce.ghost, 0.45)),
            R && (U = Ce.selected),
            E.push({ fileId: g.id, target: Jr(q), color: U }),
            i.add(g.id));
        }
      }
      for (const [g, y] of k)
        y > 0.04 && !i.has(g)
          ? E.push({ fileId: g, target: 0, color: Ce.unvisited })
          : y <= 0.04 && !i.has(g) && k.delete(g);
      (E.forEach((g, y) => A.setColorAt(y, g.color)),
        (A.count = E.length),
    ...
    r.useEffect(() => {
      if (!e || !s) return;
      const A = e.files.find((g) => g.path === s),
        x = w.current,
        k = I.current,
        E = S.current?.domElement;
      if (!A || !x || !k || !E) return;
      k.autoRotate = !1;
      const i = c.current.get(A.id) ?? Ge,
        h = vt(A, G);
      ((h.y = i), An(x, k, h, E.clientWidth, E.clientHeight, Kr));
    }, [e, G, s]),
    r.useEffect(() => {
      const A = p.current;
      if (!A || !e) return;
      ...
      if (E) {
        ...
        if (i) {
          const h = vt(i, G);
          (E.position.set(h.x, k(i) + 1.6, h.z), (E.visible = !0));
        } else E.visible = !1;
      }
      A.update(
        x.map((i) => {
          const h = vt(i, G);
          return ((h.y = k(i) + 0.4), h);
        }),
      );
    }, [e, n, G]),
...
function na(e) {
  const n = {
    ...
    s = [...e].sort((p, j) => (p.path < j.path ? -1 : 1));
  for (const p of s) {
    const j = p.path.split("/").filter(Boolean);
    let $ = n;
    for (const C of j.slice(0, -1)) {
      let L = $.children.get(C);
      (L ||
        ((L = {
          name: C,
          path: $.path ? `${$.path}/${C}` : C,
  ...
  const o = (p) => {
    ...
    for (const j of p.children.values()) p.leafCount += o(j);
    return p.leafCount;
  };
  o(n);
  const l = Math.max(n.leafCount, 1),
    ...
    b = (p, j) => ({ x: p * Math.cos(j), z: p * Math.sin(j) }),
    c = (p, j, $, C) => {
      const L = p < 1e-6 ? C : j;
      let P = C - L;
  ...
  const w = (p) => {
    ...
    const j = [...p.children.values()].sort((C, L) => (C.name < L.name ? -1 : 1)),
      $ = [...p.files].sort((C, L) => (C.path < L.path ? -1 : 1));
    for (const C of j) w(C);
    for (const C of $) {
      const L = (S + 0.5) * v;
      S += 1;
      const P = b((p.depth + 1) * m, L);
      (d.leaf.set(C.id, P),
        d.edges.push({ childFileId: C.id, points: c(p.depth * m, p.angle, (p.depth + 1) * m, L) }));
    }
    if (p.path !== "") {
  ...
  const I = new Map(),
    O = (p) => {
      I.set(p.path, p);
      for (const j of p.children.values()) O(j);
    };
  O(n);
  for (const p of d.dirs) {
    const j = I.get(p.path),
      $ = p.path.includes("/") ? p.path.slice(0, p.path.lastIndexOf("/")) : "",
      C = I.get($);
    d.edges.push({ childPath: p.path, points: c(C.depth * m, C.angle, j.depth * m, j.angle) });
  }
  const T = new Map(d.dirs.map((p) => [p.path, p]));
  for (const p of e) {
    const j = d.leaf.get(p.id);
    if (!j) continue;
    ...
    for (let L = 0; L < $.length - 1; L++) {
      C = C ? `${C}/${$[L]}` : $[L];
      const P = T.get(C);
      P && (P.radius = Math.max(P.radius, Math.hypot(j.x - P.x, j.z - P.z)));
    }
  }
  return d;
}
...
function la({ city: e, playback: n, selectedPath: s, onSelect: a, onCanvasReady: o }) {
  ...
  x.current = n;
  const k = r.useRef(null),
    E = r.useMemo(() => (e && e.files.length > 0 ? na(e.files) : null), [e]);
  return (
    r.useEffect(() => {
      const i = l.current;
      if (!i) return;
      ...
      (y.position.set(60, 110, 90), (j.current = y));
      const R = new cn({ antialias: !0 });
      (R.setPixelRatio(Math.min(window.devicePixelRatio, 2)),
        R.setSize(i.clientWidth, i.clientHeight),
        (p.current = R),
        i.appendChild(R.domElement),
        o?.(R.domElement));
      const q = new ln(y, R.domElement);
      ((q.enableDamping = !h),
      ...
      const le = 18,
        ...
        z = (H) => {
          const N = I.current;
          if (!N || !j.current || !p.current) return;
          const _ = p.current.domElement.getBoundingClientRect(),
            V = H.clientX - _.left,
            ee = H.clientY - _.top;
          let ce,
            fe = le * le;
          for (const Z of w.current) {
            ...
            if (!ke || (F.set(ke.x, Le, ke.z).project(j.current), F.z > 1)) continue;
            const ge = ((F.x + 1) / 2) * _.width,
              Me = ((1 - F.y) / 2) * _.height,
              Re = (ge - V) * (ge - V) + (Me - ee) * (Me - ee);
            Re < fe && ((fe = Re), (ce = Z));
          }
          return ce;
        };
      let D = null;
      const Y = (H) => {
          D = { x: H.clientX, y: H.clientY };
        },
        re = (H) => {
          if (!D) return;
          const N = Math.hypot(H.clientX - D.x, H.clientY - D.y);
          ((D = null), !(N > 5) && a(z(H)?.path));
        };
      let W = 0;
      const ue = (H) => {
          if (D) {
          ...
          W ||
            (W = requestAnimationFrame(() => {
              W = 0;
              const N = z(H);
              if (((R.domElement.style.cursor = N ? "pointer" : ""), !N)) {
                U.hide();
                return;
              }
              const _ = x.current,
                V = _.touchByPath.get(N.path),
                ee = _.visitsByFile.get(N.id) ?? 0,
                ce = V ? `${Xe(V)} · ${ee} visit${ee === 1 ? "" : "s"}` : Xe(void 0);
              U.show(N.path, N.ghost ? `${ce} · ghost` : ce, H.clientX, H.clientY);
            }));
        },
        se = () => {
          (U.hide(), (R.domElement.style.cursor = ""));
        };
      (R.domElement.addEventListener("pointerdown", Y),
        R.domElement.addEventListener("pointerup", re),
        R.domElement.addEventListener("pointermove", ue),
        R.domElement.addEventListener("pointerleave", se));
      const xe = () => {
          if (!l.current) return;
          const H = l.current.clientWidth,
        ...
        ie = () => {
          ...
          if (H && V && N.length > 0) {
            ...
            for (let fe = 0; fe < N.length; fe++) {
              const Z = N[fe],
                ke = V.leaf.get(Z.fileId);
              if (!ke) continue;
              let ge = _.get(Z.fileId) ?? 0;
              const Me = Z.target - ge;
              (Math.abs(Me) > 0.015
                ? ((ge = A.current ? Z.target : ge + Me * 0.12), _.set(Z.fileId, ge), (ce = !0))
                : ge !== Z.target && (_.set(Z.fileId, Z.target), (ge = Z.target), (ce = !0)),
                J.compose(
            ...
            ce && (H.instanceMatrix.needsUpdate = !0);
          }
          const ee = ne.current;
          if (ee?.visible) {
            const ce = X.getElapsedTime(),
              fe = ee.userData.baseScale,
              Z = A.current ? 1 : 1 + 0.1 * Math.sin(ce * 2.4);
            ee.scale.setScalar(fe * Z);
          }
          (R.render(g, y), (G.current = requestAnimationFrame(ie)));
        };
      return (
        ie(),
        () => {
          (G.current && cancelAnimationFrame(G.current),
            ...
            R.domElement.removeEventListener("pointerdown", Y),
            R.domElement.removeEventListener("pointerup", re),
            R.domElement.removeEventListener("pointermove", ue),
            R.domElement.removeEventListener("pointerleave", se),
            U.dispose(),
            je.disconnect(),
            q.dispose(),
            R.dispose(),
    ...
    r.useEffect(() => {
      ...
      const R = new pn(g * 2.4, 40, "#1d222c", "#181c25");
      ((R.material.transparent = !0), (R.material.opacity = 0.4), (R.position.y = -0.24), h.add(R));
      const q = [],
        U = [];
      for (const N of E.edges) {
        let _ = 0;
        for (let V = 0; V < N.points.length - 1; V++)
          (q.push(N.points[V].x, 0.12, N.points[V].z),
            q.push(N.points[V + 1].x, 0.12, N.points[V + 1].z),
            (_ += 2));
        U.push({ childPath: N.childPath, childFileId: N.childFileId, vertexCount: _ });
      }
      const de = new nn();
      ...
      ((c.current = le), (S.current = U), h.add(le));
      const F = new It(0.5, 10, 8),
        z = new Be({ toneMapped: !1, fog: !1 }),
        D = new qe(F, z, e.files.length);
      D.instanceColor = new st(new Float32Array(e.files.length * 3), 3);
      const Y = e.files.filter((N) => N.ghost),
        re = new Map();
      (Y.forEach((N, _) => re.set(N.id, _)), (v.current = re));
      const W = new _e(),
        ue = new _e().makeScale(0, 0, 0);
      for (const N of e.files) {
        const _ = E.leaf.get(N.id);
        if (!_) continue;
        if (N.ghost) {
          D.setMatrixAt(N.id, ue);
          continue;
        }
        const V = Math.min(0.24 + Math.sqrt(Math.max(N.lines, 1)) * 0.045, 1.05);
        (W.compose(new te(_.x, Le, _.z), new ze(), new te(V, V, V)),
          D.setMatrixAt(N.id, W),
          D.setColorAt(N.id, Ne.unvisited));
      }
      ((D.instanceMatrix.needsUpdate = !0),
        D.instanceColor && (D.instanceColor.needsUpdate = !0),
        (f.current = D),
      ...
      if (Y.length > 0) {
        ((se = new qe(
          ...
          (se.instanceColor = new st(new Float32Array(Y.length * 3), 3)));
        for (const N of Y) {
          const _ = E.leaf.get(N.id),
            V = re.get(N.id);
          if (!_) {
          ...
          (W.compose(new te(_.x, Le, _.z), new ze(), new te(ee, ee, ee)),
            se.setMatrixAt(V, W),
            se.setColorAt(V, Ne.ghost));
        }
      ...
      const ie = new zn(1.6);
      ((P.current = ie), h.add(ie.object), (L.current = h), i.add(h));
      const H = () => {
        const N = j.current,
          _ = $.current;
        if (!N || !_) return !0;
        const V = new te(0.3, 0.66, 0.47).normalize(),
          ee = [...E.leaf.values()].map((Z) => new te(Z.x, Le, Z.z)),
          ce = $n(N, V, ee);
        if (ce === null) return !1;
        const fe = ce * 1.12;
        return (
          N.position.copy(V).multiplyScalar(fe),
          _.target.set(0, 0, 0),
          (_.minDistance = g * 0.15),
          (_.maxDistance = Math.max(g * 2.4, fe * 1.2)),
          _.update(),
          !0
        );
      };
    ...
    r.useEffect(() => {
      ...
      for (const z of e.files) {
        const D = n.touchByFile.get(z.id);
        let Y = z.ghost ? Ne.ghost : Ne.unvisited;
        if (D) {
          Y = Ne[D];
          const re = n.visitsByFile.get(z.id) ?? 1;
          (U.push({ fileId: z.id, target: oa(re), color: Ne[D] }), de.add(z.id));
          const W = z.path.split("/");
          let ue = "";
          for (let se = 0; se < W.length - 1; se++)
            ((ue = ue ? `${ue}/${W[se]}` : W[se]), ae.add(ue));
        }
        if (z.ghost) {
          const re = g.get(z.id);
          h && re !== void 0 && h.setColorAt(re, Y);
        } else i.setColorAt(z.id, Y);
      }
      for (const [z, D] of q)
        D > 0.04 && !de.has(z)
          ? U.push({ fileId: z, target: 0, color: Ne.unvisited })
          : D <= 0.04 && !de.has(z) && q.delete(z);
      (U.forEach((z, D) => y.setColorAt(D, z.color)),
        (y.count = U.length),
        ...
        (O.current = U));
      const le = R.geometry.getAttribute("color");
      let F = 0;
      for (const z of S.current) {
        let D = !1;
        z.childFileId !== void 0
          ? (D = n.touchByFile.has(z.childFileId))
          : z.childPath && (D = ae.has(z.childPath));
        const Y = D ? ra : sa;
        for (let re = 0; re < z.vertexCount; re++) le.setXYZ(F++, Y.r, Y.g, Y.b);
      }
      le.needsUpdate = !0;
    }, [e, E, n]),
    r.useEffect(() => {
      ...
      if (g) {
        ...
        y &&
          R &&
          q &&
          ((R.autoRotate = !1), An(y, R, new te(g.x, Le, g.z), q.clientWidth, q.clientHeight, ca));
    ...
    r.useEffect(() => {
      ...
      const h = n.recentTargets
          ...
          .filter((y) => !!(y && E.leaf.get(y.id))),
        g = ne.current;
      if (g) {
        const y = h[h.length - 1];
        if (y) {
          const R = E.leaf.get(y.id);
          (g.position.set(R.x, Le + 1.7, R.z), (g.visible = !0));
        } else g.visible = !1;
      }
      i.update(
        h.map((y) => {
          const R = E.leaf.get(y.id);
          return new te(R.x, Le + 0.3, R.z);
        }),
      );
    }, [e, E, n]),
...
const Pe = pa((e, n) => ({
  ...
  ga = r.memo(function ({
    ...
    const c = n?.stats,
      ...
      I = c ? Math.max(0, c.filesInRepo - c.fovea - c.parafovea) : 0,
      O = s ? s.files.reduce((P, ne) => P + (ne.ghost ? 1 : 0), 0) : 0,
      T = c ? Xt(c.errors) : 0,
    ...
    return (
      ...
      r.useEffect(() => {
        ...
        const P = (K) => {
            const G = K.target;
            C.current?.contains(G) || L.current?.contains(G) || $(!1);
          },
          ne = (K) => {
            K.key === "Escape" && $(!1);
          };
                      ...
                      ? t.jsxs("div", {
                          ...
                          children: [
                            ...
                            c.churnFiles > 0
                              ? t.jsxs("button", {
                                  ref: L,
                                  className: j ? "warn churn-toggle open" : "warn churn-toggle",
                                  "aria-expanded": j,
                                  onClick: () => $((P) => !P),
                            ...
                            m.slice(0, yt).map((P) =>
                              t.jsxs(
                                "button",
                                {
                                  ...
                                  children: [
                                    t.jsx("span", { className: "churn-path", children: P.path }),
                                    t.jsxs("span", {
                                      className: "churn-count",
                                      children: [P.edits, " edit", P.edits === 1 ? "" : "s"],
                                    }),
                                  ],
                                },
                                P.path,
...
const Na = typeof navigator < "u" && /Mac|iPhone|iPad/.test(navigator.platform),
  ...
  Ca = r.memo(function ({
    ...
    r.useEffect(() => {
      if (!C) return;
      const x = (E) => {
          P.current?.contains(E.target) || L(!1);
        },
        k = (E) => {
          E.key === "Escape" && L(!1);
        };
    ...
    const ne = r.useMemo(() => [...new Set(n.map((x) => x.harness))].sort(), [n]),
      K = r.useMemo(() => n.filter((x) => x.eventCount === 0).length, [n]),
      G = l && ne.includes(l) ? l : void 0,
      A = r.useMemo(() => {
        const x = T.trim().toLowerCase();
        return n.filter((k) =>
          Fn(k, { hideEmpty: o, harness: G }, s)
            ? x
              ? `${k.title ?? ""} ${k.id} ${k.gitBranch ?? ""} ${k.harness}`
                  .toLowerCase()
                  .includes(x)
              : !0
            : !1,
                          ...
                          children: [
                            w
                              ? t.jsxs("button", {
                                  ...
                                  children: [
                                    t.jsx(Tt, { size: 14, "aria-hidden": !0 }),
                                    t.jsxs("span", {
                            ...
                            t.jsxs("form", {
                              ...
                              children: [
                                t.jsx("input", {
                                  ...
                                  value: j,
                                  onChange: (x) => $(x.currentTarget.value),
                                  spellCheck: !1,
                                }),
                                t.jsx("button", {
                                  type: "submit",
                          ...
                          children: [
                            ...
                            ne.map((x) =>
                              t.jsx(
                                "button",
                                {
                                  className: G === x ? "chip active" : "chip",
                                  onClick: () => b(x),
                                  children: Qt(x),
                                },
                                x,
                  ...
                  children: [
                    ...
                    t.jsxs("span", {
                      ...
                      children: [
                        ...
                        (() => {
                          const k = x.key === s && O !== void 0 ? O : x.reportState;
                          return k
                            ? t.jsx("span", {
                                className: `rail-eval rail-eval-${k}`,
                                title: Zt(k),
                                "aria-label": Zt(k),
                                children: k === "running" ? "evaluating" : "",
                              })
                            : null;
                        })(),
                      ],
    ...
    r.useEffect(() => {
      ...
      const i = Math.max(85, en / v),
        ...
        g = window.setInterval(() => {
          ...
          s(Math.min(p.current + h, j.current));
        }, i);
      return () => window.clearInterval(g);
    }, [f, v, w, s, l]));
  const C = r.useCallback(() => {
      (!$.current && p.current >= j.current && s(0), m((i) => !i));
    }, [s]),
    L = r.useCallback(() => {
    ...
    P = r.useCallback(
      (i) => {
        s(Math.min(j.current, Math.max(0, p.current + i)));
      },
      [s],
    ),
    ne = r.useCallback(
      ...
      if (
        ...
        switch (h.key) {
          case " ":
            ...
            return;
          case "ArrowLeft":
            (h.preventDefault(), P(h.shiftKey ? -10 : -1));
            return;
          case "ArrowRight":
            (h.preventDefault(), P(h.shiftKey ? 10 : 1));
            return;
        ...
        switch (h.key.toLowerCase()) {
          case "s":
            ...
            return;
          case "e":
            ne(h.shiftKey ? -1 : 1, (y) => y.action === "edit");
            return;
          case "x":
            ne(h.shiftKey ? -1 : 1, (y) => y.isError);
            return;
          case "m":
            G(h.shiftKey ? -1 : 1);
            return;
  ...
  const x = r.useMemo(() => {
      ...
      for (let g = 0; g < i; g++) {
        const y = Math.floor((g * w) / i),
          ...
          q = new Map();
        for (let le = y; le < R; le++) {
          const F = e.events[le].action;
          q.set(F, (q.get(F) ?? 0) + 1);
        }
        let U = "other",
          de = -1;
        ...
        h.push({ count: R - y, dominant: U });
      }
      return h;
    }, [e, w]),
    k = r.useMemo(() => x.reduce((i, h) => Math.max(i, h.count), 1), [x]),
    E = r.useMemo(() => {
      if (!e) return [];
      const i = new Map();
      for (const h of e.marks) {
        const g = Math.min(h.seq, I),
          y = I > 0 ? g / I : 0,
          R = `${h.type}:${Math.round(y * $a)}`,
          q = i.get(R);
        q
...
function za() {
  ...
  const R = r.useRef(null),
    ...
    [le, F] = r.useState(null),
    [z, D] = r.useState(),
    [Y, re] = r.useState(),
    [W, ue] = r.useState(null),
    [se, xe] = r.useState(!1),
    [je, X] = r.useState(),
    [J, he] = r.useState(),
    [ie, H] = r.useState(),
  ...
  const ke = r.useCallback((u) => {
    ...
    Se = r.useCallback(async (u, M = k.current) => {
      const B = ++E.current;
      (xe(!0), he(void 0), H(void 0));
      try {
        const Q = await lr(u);
        if (M !== k.current || B !== E.current || y.current !== u) return;
        re(Q);
      } catch (Q) {
        M === k.current &&
          B === E.current &&
          y.current === u &&
          (he(Ae(Q, "loading agents")), H(null));
      } finally {
        M === k.current && B === E.current && y.current === u && xe(!1);
      }
    }, []),
    Je = r.useCallback(
      async (u) => {
        ...
        try {
          ...
          if (M !== x.current || B !== k.current || y.current !== u) return;
          ((V.current = Q), (ee.current = ve), ce.current.set(Ie, Q));
          const ye = N.current;
          if (ye === null) {
            T(Q, ve);
            const we = fe.current.get(Ie);
            (we !== void 0 && j(Math.min(we, Math.max(0, Q.events.length - 1))), $(void 0));
          } else {
            const we = ce.current.get(ye);
            if (we) {
              const pe = Pe.getState().currentSeq;
              (T(we, ve), j(Math.min(pe, Math.max(0, we.events.length - 1))));
            }
          }
        } catch (Q) {
          M === x.current && y.current === u && L(Ae(Q, "loading the session"));
        } finally {
          Me();
        }
      },
    ...
    Et = r.useCallback(() => {
      ...
      if (M && B) {
        ...
        (Q !== void 0 && j(Math.min(Q, Math.max(0, M.events.length - 1))), $(void 0));
      }
    }, [j, T, $]),
    lt = r.useCallback(
      async (u) => {
        ...
        try {
          ...
          if (!G.current) {
            ...
            const be = new URL(window.location.href).searchParams.get("session") ?? void 0,
              At = be ? B.find((xt) => xt.key === be) : void 0,
              gt = be && !At ? B.filter((xt) => xt.id === be) : [],
          ...
          const ve = y.current,
            ye = ve !== void 0 && B.some((be) => be.key === ve),
            we = (B.find((be) => Fn(be, { hideEmpty: d, harness: b })) ?? B[0])?.key,
            pe = Q ?? (ye ? ve : we);
          if (pe !== ve) {
            const be = Re();
            ((y.current = pe), pe || x.current++, O(pe), pe && Se(pe, be));
          } else u && pe && (Et(), Se(pe));
          pe && (await Je(pe));
        } catch (B) {
          M === A.current && L(Ae(B, "scanning sessions"));
        } finally {
          Me();
        }
      },
      [ge, Me, b, d, Et, Se, Je, Re, O, L, I],
    ),
    Dn = r.useCallback(
      async (u) => {
        (ge(), L(void 0));
        try {
          ...
          L(Ae(M, "loading the repository map"));
        } finally {
          Me();
        }
      },
    ...
    On = r.useCallback((u) => {
      const M = u ? `/?map=1&repo=${encodeURIComponent(u)}` : "/?map=1";
    ...
    ut = r.useCallback(
      async (u) => {
        ...
        if (!M || !B) return;
        const Q = ++i.current;
        if (((_.current = void 0), X(void 0), he(void 0), H(void 0), N.current === u)) return;
        const ve = u === null ? V.current : ce.current.get(u);
        if (ve) {
          dt(u, ve, B);
          return;
        }
        ...
        try {
          ...
          if (
            ...
            return;
          (ce.current.set(u, pe), dt(u, pe, ee.current ?? B));
        } catch (pe) {
          we === k.current &&
            Q === i.current &&
            _.current === u &&
            y.current === M &&
            (he(Ae(pe, `loading the ${ye.label} trace`)), H(u));
        } finally {
          we === k.current &&
            Q === i.current &&
            _.current === u &&
            y.current === M &&
            ((_.current = void 0), X(void 0));
        }
      },
      [Y, dt],
    ),
    ...
    Vn = r.useCallback(async () => {
      const u = R.current,
        M = s?.events.length ?? 0;
      if (!u || M === 0 || Z.current) return;
      const B = y.current,
        Q = N.current,
        ve = Pe.getState().currentSeq;
      (i.current++, (_.current = void 0), X(void 0), (Z.current = !0), U(!0), L(void 0));
      try {
        const { blob: ye, extension: we } = await qr({ canvas: u, total: M, setSeq: j }),
          pe = s?.session.id || B || "session";
        Dr(ye, `mindwalk-${pe}.${we}`);
      } catch (ye) {
        L(Ae(ye, "exporting the video"));
      } finally {
        (y.current === B && N.current === Q && j(ve), (Z.current = !1), U(!1));
      }
    }, [s, q, j, L]),
    Gn = r.useCallback(() => K(!0), [K]),
    Kn = r.useCallback(() => K(!1), [K]),
  ...
  r.useEffect(() => {
    (ae(null), F(null), D(void 0), n && !S && Ee(n));
  }, [n, S, Ee]);
  ...
  const Yn = r.useCallback(
      async (u) => {
        ...
        if (M)
          try {
            const B = await hr(M, u);
            (y.current === M && D(B), $e());
          } catch (B) {
            L(Ae(B, "starting the evaluation"));
          }
      },
      [L, $e],
    ),
    ...
    Xn = r.useCallback(() => F(null), []),
    Jn = r.useCallback(() => {
      Z.current || ae("agents");
    }, []),
    Zn = r.useCallback((u) => {
      u.presentation === "sheet"
        ? ae((M) => (M === u.id ? null : u.id))
        : F((M) => (M === u.id ? null : u.id));
    }, []),
    Qn = r.useCallback(
      (u) => {
        if (Z.current) return;
        ...
        (i.current++,
          (_.current = void 0),
          X(void 0),
          N.current !== null && (Ze(), (N.current = null), ue(null), T(M, B)),
          j(u),
          fe.current.set(Ie, u));
        const ve = M.events[u]?.targets.find((ye) => ye.path)?.path;
        $(ve);
      },
      [Ze, j, T, $],
    ),
  ...
  (r.useEffect(() => {
    const u = (M) => {
      ...
      M.preventDefault();
      const B = Pe.getState();
      B.setRailCollapsed(!B.railCollapsed);
    };
    ...
    r.useEffect(() => {
      const u = (M) => {
        ...
        const B = M.target;
        if (
          (B && (B.tagName === "INPUT" || B.tagName === "TEXTAREA" || B.isContentEditable)) ||
          Z.current
        )
          return;
        const Q = Pe.getState();
  ...
  const pt = r.useMemo(
    ...
    St = r.useMemo(
      () => (W === null ? "Main" : (Y?.agents.find((u) => u.id === W)?.label ?? "Subagent")),
      [W, Y],
    ),
    ns =
      f === "tree"
    ...
    Te = r.useMemo(() => $t.snapshotAt(o), [$t, o]),
    mt = r.useMemo(() => {
      let u = 0,
        M = 0,
        B = 0;
      for (const Q of Te.touchByPath.values()) Q === "edit" ? u++ : Q === "read" ? M++ : B++;
      return { edited: u, read: M, seen: B };
    }, [Te]),
    Oe = r.useMemo(() => (l ? a?.files.find((u) => u.path === l) : void 0), [a, l]),
    ss = r.useMemo(() => {
      const u = new Map();
      for (const M of s?.events ?? [])
        for (const B of M.targets)
          B.touch === "edit" && B.path && u.set(B.path, (u.get(B.path) ?? 0) + 1);
      return [...u.entries()]
        .filter(([, M]) => M >= 3)
        .map(([M, B]) => ({ path: M, edits: B }))
        .sort((M, B) => B.edits - M.edits || (M.path < B.path ? -1 : 1));
    }, [s]);
  return t.jsxs("main", {
    ...
    children: [
      S
        ...
        : t.jsx(Ca, {
            ...
            onRefresh: Wn,
            onHideEmptyChange: P,
            onHarnessFilterChange: ne,
            onCollapse: Gn,
            onOpenMap: On,
            activeRepo: s?.session.cwd,
            locked: q,
            activeReportState: z === void 0 ? void 0 : pt,
          }),
      t.jsxs("section", {
        className: "stage",
        children: [
                    ...
                    panels: [
                      ...
                      },
                      {
                        ...
                        render: () =>
                          t.jsx(ja, {
                            file: Oe,
                            touch: Oe ? Te.touchByPath.get(Oe.path) : void 0,
                            history: Oe ? (Te.historyByPath.get(Oe.path) ?? []) : [],
                            onClose: ft,
                            onJumpTo: ts,
                            locked: q,
                          }),
...
as.createRoot(document.getElementById("root")).render(
  t.jsx(r.StrictMode, { children: t.jsx(za, {}) }),
);
//# sourceMappingURL=index-BxfYFk9W.js.map
