...
func TestParseLargeMessageHistory(t *testing.T) {
	...
	createCrushDBAt(t, dbPath, func(db *sql.DB) {
		_, err := db.Exec(
			...
			messageCount, 1785585600+messageCount, 1785585600,
		)
		if err != nil {
			t.Fatal(err)
		}
		...
		if role == "assistant" {
			...
			parts = fmt.Sprintf(`[{"data":{"text":"step %d"},"type":"text"},{"data":{"finished":true,"id":"%s","input":"{\"file_path\":\"/repo/file_%d.go\"}","name":"read"},"type":"tool_call"}]`, i, callID, i)
		} else {
			parts = fmt.Sprintf(`[{"data":{"content":"ok","is_error":false,"name":"read","tool_call_id":"call_%04d"},"type":"tool_result"}]`, i-1)
		}
		ts := 1785585600 + i
		_, err := db.Exec(
			`INSERT INTO messages (id, session_id, role, parts, model, created_at, updated_at) VALUES (?, 'sess-many-msgs', ?, ?, 'test-model', ?, ?)`,
			msgID, role, parts, ts, ts,
		)
		if err != nil {
			t.Fatal(err)
		}
